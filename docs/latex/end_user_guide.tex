...
The application is a protected Flask workbench for a compact four-trade QuantLib portfolio:
\begin{itemize}[leftmargin=1.5em]
    \item a spot-starting SOFR swap,
    \item a European swaption priced from an editable ATM normal-volatility matrix,
    \item a Bermudan swaption priced with a calibrated Gaussian short-rate model, and
    \item an S\&P 500 equity cliquet with reset-strip analytics.
\end{itemize}

The main screen is \code{/dashboard}. The technical model and research page is \code{/quantlib-data-model}. Each trade has a dedicated editor at \code{/trade/<trade\_type>}.
...
\subsection{Portfolio Marks}
The top blotter shows the mark-to-market and NPV of all five trades. The blotter is the fastest way to confirm whether a market move or trade edit changed the portfolio in the expected direction. The second Bermudan is the workbook benchmark deal used for QL spreadsheet comparison.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item a zero-rate chart built from QuantLib curve node dates using continuous-compounded spot rates on an Actual/365 basis,
    \item a ten-year daily one-day forward SOFR chart,
...
The European swaption editor controls direction, notional, strike, expiry, underlying swap tenor, payment frequency, and reset frequency. The trade reads exactly one volatility cell from the dashboard matrix.

\subsection{Bermudan Swaption}
The Bermudan editor controls direction, notional, strike, first exercise date, fixed final maturity, payment frequency, and reset frequency. Exercise opportunities are generated on the payment schedule from first exercise up to the period before final maturity. The pricing engine calibrates a time-dependent Gaussian short-rate sigma term structure to the feasible diagonal of the matrix before pricing, while keeping the user-specified Hull-White mean reversion fixed. A \code{5Y NC 2Y} trade therefore keeps the same final maturity date and steps through the remaining \code{3Y}, \code{2Y}, and \code{1Y} swaps as the annual exercise dates arrive.
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item Review the blotter, zero-rate panel, forward strip, Bermudan grid, and cliquet analytics.
...
Swaption risk is slow & Bermudan bump-and-reprice analytics rebuild and recalibrate the Gaussian short-rate model many times. This is expected. \\
