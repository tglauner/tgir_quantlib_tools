...
\hypersetup{
  pdftitle={Callable Bermudan Cross-Currency Swap - Quantitative Model Summary (V2)},
...
\title{Callable Bermudan Cross-Currency Swap - Quantitative Model Summary (V2)}
\TGIRSetArticlePublicationDate{September 5, 2026}
\author{\TGIRArticleAuthorBlock}

\begin{document}
\maketitle

\noindent\textbf{Model:} two Hull-White rate factors + lognormal FX + Bermudan LSM\\
\textbf{Numeraire:} domestic money-market account; values in domestic currency\\
\textbf{Perspective:} positive cash flows are receipts by the exercise holder

\section{Product and notation}
...
\section{Joint stochastic model}
...
\begin{align}
  ...
  \dd r_f(t) &= \bigl[\theta_f(t)-a_fr_f(t)\bigr]\dd t
                 -\rho_{fS}\sigma_f(t)\sigma_S(t)\dd t \nonumber\\
              &\quad +\sigma_f(t)\dd W_f(t), \\
...
\section{Monte Carlo discretization}
...
\section{Bermudan exercise by least-squares Monte Carlo}
...
\section{Measures, risks, and validation}
...
\section{Known limitations, roadmap, and feedback}

\textbf{Known limitations and roadmap.} This summary describes a transparent research reference model, not a production or independently validated pricing model. Domestic and foreign short-rate volatility is currently one-piece constant per currency rather than a fitted term structure, the FX volatility input is ATM/benchmark-tenor only with no smile or skew calibration, forecast-discount and cross-currency basis are modeled deterministically rather than as stochastic risk factors, and the reported Bermudan value is a Longstaff-Schwartz lower bound with no companion dual upper bound to size the estimator bias directly. Bump-and-recalibrate rate vega is also known to be noisy at moderate Monte Carlo path counts and is not yet a stable production risk number. The roadmap for the next model version prioritizes, in order: (1) a full term-structure calibration of domestic and foreign Hull-White volatility rather than a single blended parameter per currency; (2) FX smile/skew calibration in place of the current ATM-only fit; (3) a more stable vega methodology, together with a dual-bound or nested Monte Carlo estimate of the LSM lower-bound bias; and (4) an explicit stochastic cross-currency-basis extension. Sequencing may change as review priorities are confirmed.

\textbf{Feedback.} This is a working document, and we are actively requesting feedback from quantitative and model-validation reviewers, including senior stakeholders at counterparty institutions, on the model specification, calibration approach, numerical methodology, risk coverage, and validation evidence summarized above. Please direct comments to the author. Feedback received will be incorporated into subsequent revisions of this summary and the underlying reference implementation, and material comments will be acknowledged in the change history of future versions.
