...
\pagestyle{fancy}
\fancyhf{}
\rhead{Exotic Derivatives Pricing}
...
\hypersetup{
    ...
    pdfauthor={TG Investments Research}
}
...
\title{\textbf{Pricing of Complex Exotic Derivatives:\\
Callable Cross-Currency Swaps and Credit Contingent Interest Rate Swaps}\\[1em]
\large Market Standards, Tier-1 Bank Methodologies, and QuantLib Assessment}

\author{TG Investments Research\\
\small August 2026}
...
\textbf{Keywords:} Callable cross-currency swap, credit contingent swap, CVA, wrong-way risk, Hull-White, G2++, LGM model, swaption calibration, QuantLib
...
\diff r^{\text{EUR}}_t &= [\theta_{\text{EUR}}(t) - a_{\text{EUR}} r^{\text{EUR}}_t + \lambda_{\text{FX}} \sigma_{\text{EUR}} \sigma_X \rho_{r^{\text{EUR}},X}] \diff t + \sigma_{\text{EUR}} \diff W^{2}_t \\
...
\begin{equation}
V_{\text{CCS}} = \E^{\Q}\left[ \ind_{\{\tau \leq T\}} D(0,\tau) \cdot g(V_\tau) + \ind_{\{\tau > T\}} \cdot \text{(standard swap PV)} \right]
\end{equation}
...
A(t,T) &= \frac{P(0,T)}{P(0,t)} \exp\left( B(t,T) \frac{\partial \ln P(0,t)}{\partial t} - \frac{\sigma^2}{4a}(1-e^{-2at})B(t,T)^2 \right)
\end{align}
